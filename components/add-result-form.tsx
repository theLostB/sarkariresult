...
const AddResultForm: React.FC<AddResultFormProps> = ({ onSubmit, onCancel, initialValues, mode = 'add', heading }) => {
    ...
    if (mode === 'add') {
      ...
      fetch('/api/notify', {
        ...
        body: JSON.stringify({
          ...
          body: `${title || 'A new result'} has been added. Check it now!`,
          url: '/'
        })
      });
    } else {
  ...
  return (
    <div className="bg-white border rounded-lg p-4 md:p-8 shadow-lg max-w-md mx-auto mt-10">
      <h2 className="text-lg md:text-2xl font-bold mb-4 text-teal-700">{heading || (mode === 'add' ? 'Add Result' : 'Update Result')}</h2>
      <form
        onSubmit={handleSubmit}
        className="flex flex-col gap-4"
      >
        <div>
          <label className="block font-medium mb-1">Result Title</label>
          <input
            type="text"
        ...
        <div>
          <label className="block font-medium mb-1">Download Result URL</label>
          <input
            type="url"
            className="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-teal-400"
            value={url}
        ...
        <div className="flex gap-2 mt-2">
          ...
          >{mode === 'add' ? 'Add Result' : 'Save'}</button>
          <button
            type="button"
